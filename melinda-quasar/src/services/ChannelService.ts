...
class ChannelService {
    ...
    async joinByInviteCode(inviteCode: string): Promise<Channel> {
        const response = await api.post<Channel>('channels/join/invite', { inviteCode })
        return response.data
    }
    ...
    ): Promise<{ success: boolean; invited: boolean; message?: string }> {
    ...
    async kick(
        channelId: number,
        nickname: string
    ): Promise<{ success: boolean; banned: boolean; votes: number; message: string }> {
    ...
    async searchPublic(query: string): Promise<Channel[]> {
        const response = await api.get<Channel[]>('channels/public/search', {
