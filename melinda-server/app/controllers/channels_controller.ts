...
import { emitToUser, emitMemberCountUpdate, emitNotification } from '#services/ws_events'
...
export default class ChannelsController {
    ...
    private async deleteInactiveChannels() {
        ...
        for (const channel of inactive) {
            await this.cleanupAndDeleteChannel(channel, 'inactive')
        }
    }

    private async cleanupAndDeleteChannel(channel: Channel, reason: 'admin' | 'inactive', adminUser?: User) {
        ...
        for (const member of members) {
            try {
                const notif = await Notification.create({
                    ...
                    type: reason === 'admin' ? 'channel_deleted_admin' : 'channel_deleted_inactive',
                    isRead: false,
                })

                await notif.load('sender')
                await emitNotification(member.id, notif.serialize())
    ...
    public async index({ auth }: HttpContext) {
        ...
        for (const c of memberChannels) {
            ...
            map.set(c.id, { ...serialized, isMember: true, isInvited: false, isAdmin: c.adminId === user.id })
        }
        ...
        for (const c of invitedChannels) {
            ...
            if (existing) {
                ...
                const serialized = c.serialize() as Omit<ChannelResponse, 'isMember' | 'isInvited' | 'isAdmin'>
                map.set(c.id, { ...serialized, isMember: false, isInvited: true, isAdmin: c.adminId === user.id })
            }
        }
        return [...map.values()]
    }

    public async searchPublic({ auth, request }: HttpContext) {
        const user = auth.user!
        const qRaw = request.input('q') as string | undefined
        const q = qRaw ? qRaw.trim() : ''

        const baseQuery = Channel.query()
            .where('visibility', 'public')
            .whereDoesntHave('members', (membersQuery) => {
                membersQuery.where('users.id', user.id)
            })
            .whereDoesntHave('invitations', (invQuery) => {
                invQuery.where('users.id', user.id)
            })
        ...
        const channels = await baseQuery
            .orderBy('n_members', 'desc')
            .orderBy('name', 'asc')
            .limit(50)
    ...
    public async store({ auth, request, response }: HttpContext) {
        ...
        if (iconImage) {
            if (!iconImage.isValid) return response.badRequest({ error: 'Invalid image file' })
            const fileName = `${new Date().getTime()}_${iconImage.clientName}`
            await iconImage.move(app.makePath('public/avatars/channels'), { name: fileName })
        ...
        return { ...serialized, isMember: true, isInvited: false, isAdmin: true }
    }
    ...
    public async joinByName({ auth, request, response }: HttpContext) {
        ...
        if (!existing) {
            ...
            return { ...serialized, isMember: true, isInvited: false, isAdmin: true } as ChannelResponse
        }

        const channel = existing
        const banned = await channel.related('blacklist').query().where('users.id', user.id).first()
        if (banned) return response.forbidden({ error: 'You are banned from this channel' })
        ...
        if (channel.visibility === 'private') {
            ...
            if (!isMemberCheck && !hasInvitation) return response.forbidden({ error: 'Cannot join a private channel without invitation' })
        }
        ...
        if (isMember) {
            const serialized = channel.serialize()
            return { ...serialized, isMember: true, isInvited: false, isAdmin: channel.adminId === user.id } as ChannelResponse
        }
    ...
    public async joinByInvite({ auth, request, response }: HttpContext) {
        const user = auth.user!
        const inviteCode = (request.input('inviteCode') as string | undefined)?.trim()

        if (!inviteCode) {
            return response.badRequest({ error: 'Invite code is required' })
        }

        const channel = await Channel.findBy('inviteCode', inviteCode)
        if (!channel) {
    ...
    public async decline({ auth, params, response }: HttpContext) {
        ...
        const invitation = await channel.related('invitations').query().where('users.id', user.id).first()
        if (!invitation) return response.notFound({ error: 'Invitation not found' })
        await channel.related('invitations').detach([user.id])
        return { success: true }
    }
    ...
    public async leave({ auth, params, response }: HttpContext) {
        ...
        if (channel.adminId === user.id) {
            await this.cleanupAndDeleteChannel(channel, 'admin', user)
            return { success: true, deleted: true }
        }

        const isMember = await channel.related('members').query().where('users.id', user.id).first()
        if (!isMember) return response.forbidden({ error: 'You are not a member of this channel' })
    ...
    public async destroy({ auth, params, response }: HttpContext) {
        const user = auth.user!
        const channel = await Channel.findOrFail(params.id)
        if (channel.adminId !== user.id) return response.forbidden({ error: 'Only channel admin can delete the channel' })
        await this.cleanupAndDeleteChannel(channel, 'admin', user)
        return { success: true }
    }
    ...
    public async invite({ auth, params, request, response }: HttpContext) {
        ...
        if (channel.visibility === 'private' && channel.adminId !== user.id) return response.forbidden({ error: 'Only channel admin can invite to a private channel' })
        if (channel.visibility !== 'private') {
            const isMember = await channel.related('members').query().where('users.id', user.id).first()
        ...
        if (banned) {
            if (channel.adminId === user.id) await channel.related('blacklist').detach([target.id])
            else return response.forbidden({ error: 'User is banned. Only admin can restore access.' })
        }
        ...
        if (alreadyInvited) return { success: true, invited: false, message: 'User is already invited' }
        ...
        await emitNotification(target.id, notif.serialize())

        await emitToUser(target.id, 'channel:invite', { channelId: channel.id, channelName: channel.name })
    ...
    public async kick({ auth, params, request, response }: HttpContext) {
        ...
        if (channel.adminId === user.id) {
            ...
            if (channel.visibility === 'private') return response.forbidden({ error: 'Only admin can kick in private channels' })
    ...
    public async ban({ auth, params, request, response }: HttpContext) {
        ...
        const channel = await Channel.findOrFail(params.id)
        const payload = await request.validateUsing(ChannelNicknameActionValidator)
        if (channel.adminId !== user.id) return response.forbidden({ error: 'Only channel admin can ban users' })
        let target: User
        try {
            target = await this.findUserByNicknameOrFail(payload.nickname)
        } catch {
        ...
        const alreadyBanned = await channel.related('blacklist').query().where('users.id', target.id).first()
        if (!alreadyBanned) await channel.related('blacklist').attach([target.id])
        const wasMember = await channel.related('members').query().where('users.id', target.id).first()
        await channel.related('members').detach([target.id])
        await channel.related('invitations').detach([target.id])
    ...
    public async unban({ auth, params, request, response }: HttpContext) {
        ...
        const channel = await Channel.findOrFail(params.id)
        const payload = await request.validateUsing(ChannelNicknameActionValidator)
        if (channel.adminId !== user.id) return response.forbidden({ error: 'Only channel admin can unban users' })
        let target: User
        try {
            target = await this.findUserByNicknameOrFail(payload.nickname)
        } catch {
    ...
    public async members({ auth, params, response }: HttpContext) {
        ...
        const membership = await channel.related('members').query().where('users.id', user.id).first()
        if (!membership) return response.forbidden({ error: 'You are not a member of this channel' })
        const members = await channel.related('members').query().orderBy('nickname', 'asc')
